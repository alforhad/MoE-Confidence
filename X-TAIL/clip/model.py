...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, text_or_image=None):
        ...
        self.t_or_i = text_or_image
        self.adaptmlp_list = nn.ModuleList()
        
        if text_or_image == 'image' or text_or_image == 'text': 
            for _ in range(11):  # 11 Datasets
                ...
                self.adaptmlp_list.append(adaptmlp)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if global_taskid is not None:
            if self.t_or_i == 'image' or self.t_or_i == 'text':
                assert 0 <= global_taskid < len(self.adaptmlp_list), "Invalid Task ID"
                x = x + self.mlp(self.ln_2(x)) + self.adaptmlp_list[global_taskid](x, add_residual=False) 
            else:
                # If text, only apply MLP
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text, taskid, is_train):
        global global_taskid, global_is_train   #, global_sub_taskid
        
        global_taskid = taskid
        #global_sub_taskid = taskid[1]
        ...
        return logits_per_image, self.encode_image(image)#, self.encode_text(text) #logits_per_text
    
    """def forward(self, image, text, taskid, is_train, temperature=1.0):
        ...
        assert logit_scale.dim() == 0, "logit_scale should be a scalar."
        logit_scale = self.logit_scale.exp() / temperature
        logits_per_image = logit_scale * image_features @ text_features.t()
        logits_per_text = logits_per_image.t()
