...
def adaptmlp_param_matches(param_name: str, task_id: int) -> bool:
    return f"adaptmlp_list.{task_id}." in param_name


def cfg_truthy(cfg, key: str, default=False) -> bool:
    if cfg is None:
        return default
...
def get_workdir(path):
    ...
    for marker in ("MoE_Confidence", "MoE-Confidence"):
        parts = root.parts
        if marker in parts:
    ...
    for parent in (root, *root.parents):
        if (parent / "main_ddp.py").is_file() and (parent / "configs" / "class").is_dir():
            return str(parent)
