...
def br_dt_get():
    ds_name = 'entity13'
    info_dir="/mnt/DATA/ILSVRC/imagenet_class_hierarchy/modified/"
    data_dir="/mnt/DATA/ILSVRC/Data/CLS-LOC/"
    task_stat_path =  "/mnt/DATA/breeds_protocol/entity13_5_tasks.pkl"
    batch_size = 128
    classes = ["garment", "bird", "reptile", "arthropod", "mammal", "accessory", "craft", "equipment", "furniture", "instrument", "man-made structure", "wheeled vehicle", "produce"]
    ...
    for i in range(1, task_size):
        ...
        target_train_val_augment_task_loader_dict[i] = torch.utils.data.DataLoader(
            dset_target_train_val_augment_task_dict[i], batch_size=batch_size, shuffle=False,
            num_workers=16, pin_memory=True)
    ...
    for task_id in range(task_size):
        ...
        for loop in range(task_size):
            ...
            for _, (inputs, targets) in enumerate(tqdm(eval_loader)):
                ...
                assert torch.max(targets) <= 12, "Error: Found a target value greater than 12."
