...
# from clip.model_eval import build_model
...
def load_model_from_path(model_path: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=True, is_train=False):
    """Load a CLIP model from a specified path.
    Parameters
    ----------
    model_path : str
        The path to the model checkpoint containing the state_dict.
    device : Union[str, torch.device]
        The device to put the loaded model.
    jit : bool
        Whether to load the optimized JIT model (default) or more hackable non-JIT model.
